...
interface WindStatusProps {
  ...
  windDirection: string | undefined; // You might need to adjust the type based on your API response
}

const WindStatus: React.FC<WindStatusProps> = ({ windSpeed, windDirection }) => {
  if (windSpeed === undefined) {
    return <div>Wind data not available.</div>; // Or a placeholder
  }

  // You might need to convert wind direction from degrees to a human-readable format (e.g., N, S, E, W, NW, etc.)
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 flex flex-col">
      <h3 className="text-sm text-gray-700 font-semibold mb-2">Wind Status</h3>
      <div className="text-2xl font-bold">{windSpeed} km/h</div>
      {windDirectionText && (
        <div className="flex items-center text-sm text-gray-600 mt-1">
          <ArrowUpRightIcon className={`w-4 h-4 mr-1 transform rotate-[${windDirection}deg]`} />
          {windDirectionText}
        </div>
      )}
    </div>
